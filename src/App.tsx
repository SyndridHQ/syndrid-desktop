...
export function App({ bootStartedAt }: AppProps) {
  ...
  const stopTurn = useCallback(async () => {
    ...
    try {
      await appServerClient.interruptTurn({ threadId: selectedThread.id, turnId: activeTurnId });
    } catch (error) {
      setInterruptingTurn(false);
      appendRuntimeError(error, setRuntimeLogs);
    }
  ...
  return (
    <main className="app-shell">
      <header className="titlebar">
        <div className="brand-lockup"><span className="brand-mark">S</span><span className="brand-name">Syndrid</span><span className="product-tag">Desktop</span></div>
        <div className="titlebar-center">Agentic engineering workbench</div>
        <button className={`runtime-pill runtime-${runtime.phase}`} onClick={connect} type="button"><span className="status-dot" />{runtimeLabel}</button>
      </header>
      ...
      <aside className="activity-rail" aria-label="Primary navigation">
        {[["⌂", "Workspace"], ["⌘", "Agent"], ["◇", "Changes"], ["⑂", "Git"], ["⬡", "Extensions"], ["⚙", "Settings"]].map(([icon, label], index) => (
          <button className={`rail-button ${index === 1 ? "active" : ""}`} key={label} title={label} type="button"><span aria-hidden="true">{icon}</span><span className="sr-only">{label}</span></button>
        ))}
      </aside>
      ...
      <section className="workspace panel">
        ...
        <div className="conversation">
          ...
          {selectedThread ? (
            <section className="thread-preview"><div className="thread-header"><span className="eyebrow">Selected session</span><span className="live-badge">LIVE DATA</span></div><h3>{selectedThread.name || selectedThread.preview || "Untitled session"}</h3><p>{selectedThread.cwd || "No working directory reported"}</p><div className="thread-actions"><span>{activeModel ?? selectedThread.modelProvider}</span>{activeThreadId === selectedThread.id ? <span className="healthy">Active runtime session</span> : <button className="ghost-button" onClick={() => void resumeSelectedThread()} type="button">Resume session</button>}</div></section>
          ) : (
            <section className="working-card"><div className="working-line complete"><span>✓</span><div><strong>Desktop shell</strong><small>Tauri + React foundation</small></div></div><div className="working-line active"><span>●</span><div><strong>Runtime bridge</strong><small>stdio JSONL app-server supervision</small></div></div><div className="working-line"><span>○</span><div><strong>Agent workspace</strong><small>Create a real session to begin</small></div></div></section>
          )}
        ...
        <form className="composer-shell" onSubmit={(event) => { event.preventDefault(); void sendTurn(); }}>
          <textarea className="composer-input" disabled={runtime.phase !== "ready" || !selectedThread || activeThreadId !== selectedThread.id || sendingTurn} onChange={(event) => setDraft(event.target.value)} placeholder={composerHint} rows={3} value={draft} />
          <div className="composer-meta"><span>Adaptive routing</span><span>Ask first</span><span className="composer-hint">{composerHint}</span>{sendingTurn ? <button className="stop-button" type="button" disabled={!activeTurnId || interruptingTurn} onClick={() => void stopTurn()}>{interruptingTurn ? "Stopping…" : "Stop"}</button> : <button type="submit" disabled={!canSend}>Send</button>}</div>
        </form>
      </section>

      <aside className="right-panel panel">
        <div className="panel-heading"><div><span className="eyebrow">Control plane</span><h2>Runtime</h2></div></div>
        <dl className="diagnostic-grid"><div><dt>Status</dt><dd>{runtimeLabel}</dd></div><div><dt>Transport</dt><dd>stdio · JSONL</dd></div><div><dt>Process</dt><dd>{runtime.native?.state === "running" ? `${runtime.native.binary} · ${runtime.native.pid}` : "—"}</dd></div><div><dt>Platform</dt><dd>{runtime.server ? `${runtime.server.platformOs} / ${runtime.server.platformFamily}` : "—"}</dd></div><div><dt>Sessions</dt><dd>{threads.length}</dd></div><div><dt>Notifications</dt><dd>{notificationCount}</dd></div><div><dt>Active turn</dt><dd>{activeTurnId ? activeTurnId.slice(0, 8) : "—"}</dd></div><div><dt>Shell first frame</dt><dd>{shellReadyMs === null ? "measuring…" : `${shellReadyMs.toFixed(1)} ms`}</dd></div></dl>
...
function isAgentMessageDelta(value: unknown): value is AgentMessageDeltaNotification {
  return isRecord(value) && typeof value.threadId === "string" && typeof value.turnId === "string" && typeof value.itemId === "string" && typeof value.delta === "string";
}
...
function upsertAgentDelta(current: ConversationMessage[], delta: AgentMessageDeltaNotification): ConversationMessage[] {
  ...
  if (index === -1) return trimConversation([...current, { id, threadId: delta.threadId, turnId: delta.turnId, role: "assistant", text: delta.delta, streaming: true }]);
  const next = [...current];
...
function appendRuntimeError(error: unknown, setter: Dispatch<SetStateAction<string[]>>): void {
