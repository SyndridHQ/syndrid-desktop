    ...
    <main className="app-shell">
      ...
      <aside className="left-panel panel">
        <div className="panel-heading">
          ...
          <button className="icon-button" onClick={refreshThreads} type="button" title="Refresh sessions">
            ↻
          </button>
        </div>
        ...
        {runtime.phase !== "ready" ? (
          ...
          <div className="session-list">
            {threads.map((thread, index) => (
              <button className={`session-card ${index === 0 ? "selected" : ""}`} key={thread.id} type="button">
                <span className="session-status" />
                <span className="session-copy">
                  <strong>{thread.name ?? thread.title ?? "Untitled session"}</strong>
                  <small>{thread.cwd ?? thread.model ?? thread.id}</small>
                </span>
              </button>
            ))}
          </div>
          ...
          {selectedThread ? (
            <section className="thread-preview">
              <div className="thread-header">
                <span className="eyebrow">Most recent session</span>
                <span className="live-badge">LIVE DATA</span>
              </div>
              <h3>{selectedThread.name ?? selectedThread.title ?? "Untitled session"}</h3>
